...
def _get(url, params={}):
    ...
    if r.status_code != 200:
        raise RuntimeError("Could not get url {}; received code {}".format(
            url, r.status_code))
...
def _get_apod_img_url(params={}):
    ...
    if content['media_type'] != "image":
        raise RuntimeError("Non-image media type: {}".format(
            content['media_type']))
...
def get_random_picture(n_tries=5):
    for _ in range(n_tries):
        try:
            ...
            date_to_get = datetime.date.fromordinal(
                random.randint(OLDEST_DATE.toordinal(),
                               TODAY.toordinal())).isoformat()
    ...
    raise RuntimeError(
        "Unable to get a video resource after {} tries".format(n_tries))
...
def make_pretty_display(im, data, savepath):
    ...
    pretty_date = datetime.datetime.strptime(data['date'],
